...
# カメラ設定
CAMERA_DEVICE_ID: Final[int] = 0  # デフォルトカメラ（0）、複数ある場合は1, 2, ...
IMAGE_WIDTH: Final[int] = 1280  # 画像の幅（ピクセル）
IMAGE_HEIGHT: Final[int] = 720  # 画像の高さ（ピクセル）

# 撮影間隔設定
CAPTURE_INTERVAL_SECONDS: Final[int] = 5  # 5秒ごとに撮影

# ローカル保存設定
LOCAL_SAVE_DIR: Final[str] = "camera/captured_images"  # 画像保存ディレクトリ
